	...
	function sendpcm(blob){
				...
				oReq.onreadystatechange = function() {
					if(oReq.readyState == 4 && (oReq.status == 200 || oReq.status == 304)) {
                    		    $('p#tips').text('按住字母a键进行录音');
							$('p#message').text(oReq.responseText);
	                } 
				}
				oReq.open("POST", "/asr", true);
				oReq.send(blob);
            }
	//监听按键，开始录音
	...
	$("#playbtn").on("click",function(e){
		...
		$("#audio-container").append("<audio src=\"/tts?txt="+txt+"\" controls=\"controls\" autoplay=\"autoplay\"/>");
	})
